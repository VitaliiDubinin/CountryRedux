...
import { Box, Card, CardBody, CardHeader, List, Paragraph, Text, ThemeContext } from "grommet";
import { DashboardCardHeader } from "./DashboardCardHeader";

export const ItemCountList = ({ defaultItemProps, items, menuItems, title, ...rest }) => {
  const theme = useContext(ThemeContext);
  const { body, header } = theme.card;

  return (
    <Card {...rest}>
      <CardHeader pad={{ horizontal: header.pad, top: header.pad, bottom: "none" }}>
      ...
      <CardBody pad={{ horizontal: body.pad, vertical: "small" }}>
        <Box width={{ max: "medium" }}>
          <List data={items} defaultItemProps={defaultItemProps}>
            {(datum) => <ItemCount title={datum.title} description={datum.description} count={datum.count} />}
          </List>
        </Box>
      </CardBody>
    </Card>
